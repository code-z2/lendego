...
// minimal multi-token vault contract for stable coins
...
// withdrawing directly from this stable vault won't add yields
...
contract StablesVault is ERC20 {
    using Math for uint256;
    // array of five stable coins
    IERC20[5] public _assets;
    ...
    event Withdraw(address caller, address receiver, address owner, uint256 amount, uint256 shares);

    // keeps track of the user shares
    mapping(address => uint256) shareHolder;

    // sets the erc20 stables.
    constructor(IERC20[5] memory assets, string memory _name, string memory _symbol) ERC20(_name, _symbol) {
        _assets = assets;
    }
    ...
    function deposit(uint256 assets, uint256 choice) public {
        ...
        bool success = _assets[choice].transferFrom(msg.sender, address(this), assets);
     ...
     * @param owner - owner of the share. #case : can allow a user with vToken allowance to address to withdraw the stables
    ...
    function withdraw(
        ...
        require(assets <= shareHolder[owner], "cannot widthraw more than shareholder has");
        // the choice of stable must be available
        require(choice >= 0 && choice <= _assets.length - 1, "Invalid choice of Stable");

        uint256 shares = previewWithdraw(assets, choice);
        _withdraw(msg.sender, receiver, owner, assets, shares, choice);
     ...
     * @param owner - the shareHolder: in this case the owner of the shares
     * @param choice - the choice of stable to redeem.
     * @return uint256 - assets redeemed.
     */
    function redeem(uint256 shares, address receiver, address owner, uint256 choice) public virtual returns (uint256) {
    ...
    function _withdraw(
        ...
        shareHolder[owner] -= assets;
        _burn(owner, shares);
        _assets[choice].transfer(receiver, assets);
...
// convertToAssets
// mint, previewMint
// maxDeposit, maxWithdraw, maxRedeem, maxMint etc.
// Thank you!
