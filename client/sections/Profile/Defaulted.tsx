...
const DefaultedPositions = ({ address }: { address: string }) => {
  const [pagination, setPagination] = useState(0);
  const { positions, isPositionsAvailable } = useStore((state) => state);

  const filteredPosition = (start?: number, end?: number) =>
    isPositionsAvailable
      ? positions
          ?.filter(
            (el) => el.borrow.restricted && el.borrow.borrower === address
          )
          .slice(start, end)
      : positions;
...
export default DefaultedPositions;
